...
<div class="header">
    @if($setting->logo_impress != '')
        ...
        @endphp
        <img src="{{ $imageBase64 }}" alt="{{ $setting->title }}" style="max-width: 300px; height: auto;">
    @else
        <h2 style="font-size: 20px; font-weight: bold; margin: 0;">{{ $setting->title }}</h2>
    @endif
</div>
<div class="content">
    ...
    <p style="font-size: 14px; color: #555;">Saludos Cordiales,<br>{{ $setting->title }}</p>
    <p style="font-size: 14px; color: #555;">Email: {{ $setting->email }}<br>WhatsApp: {{ $setting->whatsapp }}</p>
</div>
<div class="footer">
    <div class="">
        <p class="text-sm text-gray-500">Visite nuestro sitio web: <a href="https://ifjreformas.es">https://ifjreformas.es</a></p>
    </div>
    <div class="">
